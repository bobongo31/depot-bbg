<footer class="bg-secondary text-white py-4 mt-auto footer-animated">
  <div class="container text-center">
    <div class="row justify-content-center">
      <!-- Logo à gauche -->
      <div class="col-md-2">
        <a href="/" class="text-white text-decoration-none">
          <img src="image/LOGO-GIC.png" alt="Logo" style="width: 80px; height: auto;">
        </a>
      </div>

      <!-- Colonne 2 : Infos légales -->
      <div class="col-md-3 mb-4">
        <h5>Informations légales</h5>
        <ul class="list-unstyled">
          <li><a href="/politique-utilisation" class="text-white text-decoration-none footer-link"><i class="fas fa-file-alt"></i> Politique d'utilisation</a></li>
          <li><a href="/politique-confidentialite" class="text-white text-decoration-none footer-link"><i class="fas fa-lock"></i> Politique de confidentialité</a></li>
          <li><a href="/conditions-generales" class="text-white text-decoration-none footer-link"><i class="fas fa-balance-scale"></i> Conditions générales</a></li>
          <li><a href="/mentions-legales" class="text-white text-decoration-none footer-link"><i class="fas fa-info-circle"></i> Mentions légales</a></li>
        </ul>
      </div>

      <!-- Colonne 3 : Autres liens -->
      <div class="col-md-3 mb-4">
    <h5>Autres</h5>
    <ul class="list-unstyled">
        <li>
            <a href="/foire-questions" class="text-white text-decoration-none footer-link">
                <i class="fas fa-question-circle"></i> FAQ
            </a>
        </li>

        @auth
            @php
                $services = auth()->user()->service;

                if (is_string($services)) {
                    $services = json_decode($services, true);
                }

                $services = is_array($services) ? $services : [];
            @endphp

            @if(in_array('Informatique', $services))
                <li>
                    <a href="{{ route('admin.system.dashboard') }}" class="text-white text-decoration-none footer-link">
                        <i class="fa-solid fa-server"></i> Exploitation système
                    </a>
                </li>
            @endif
        @endauth
    </ul>
</div>
    </div>
  </div>
  
  <!-- Copyright -->
  <div class="bg-dark text-center py-3 mt-auto">
    <div class="container">
      <p class="mb-0">© {{ date('Y') }} GIC v1.0.2</p>
    </div>
  </div>
</footer>
