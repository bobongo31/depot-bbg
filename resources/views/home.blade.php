...
<div class="container mt-5">
    ...
    <div class="mb-3">
        ...
        @if (auth()->user()->hasRole('superadmin') || auth()->user()->hasRole('payment_validator'))
    ...
    @if (auth()->user()->hasRole('superadmin') || auth()->user()->hasRole('payment_validator'))
        <table class="table table-striped table-bordered table-hover bg-light">
            ...
            <tbody>
                ...
                @foreach ($paiements->where('status', 'validé') as $paiement)
                    <tr id="paiement-row-{{ $paiement->id }}">
                        <!-- Récupération du nom du redevable depuis la relation avec la table 'clients' -->
                        <td>{{ $paiement->client->nom_redevable }}</td>

                        <!-- Vérification du prix de la matière avant le calcul -->
                        @php
